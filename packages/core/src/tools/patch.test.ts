...
`.trim();
    ...
    expect(result.error).toBeUndefined();
    // This is the key assertion to verify the bug fix
    expect(result.returnDisplay).toContain(
      '📊 PATCH SUMMARY: 1/2 files patched successfully.',
    );
    expect(result.returnDisplay).toContain('❌ ALL HUNKS FAILED for bad.txt');
    expect(result.returnDisplay).toContain(
