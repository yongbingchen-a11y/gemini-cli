...
/**
 * Ensures the resolved file path is within the secure working directory
 * to prevent path traversal attacks.
 * @param filepath The relative path to the file.
 * @param workingDirectory The absolute path of the secure base directory.
 * @return The resolved, safe absolute path.
 */
function getSafePath(filepath: string, workingDirectory: string): string {
  const workingDirPath = path.resolve(workingDirectory);
  const targetPath = path.resolve(workingDirPath, filepath);

  const relativePath = path.relative(workingDirPath, targetPath);
 ...
 * @param workingDirectory The secure directory for operations.
...
async function applyHunksToFile(
  ...
  const safeFilepath = getSafePath(filepath, workingDirectory);
 ...
 * @param workingDirectory The secure base directory for file operations.
